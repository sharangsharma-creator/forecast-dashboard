...
def update_dashboard(sel_mats, show_trend):
    ...
    for idx, mat in enumerate(sel_mats):
        ...
        if len(tr) > 0:
            fig.add_trace(go.Bar(x=tr['ds'], y=tr['y'], name='Actual (Train)',
        ...
        if len(te) > 0:
            fig.add_trace(go.Bar(x=te['ds'], y=te['y'], name='Actual (Test)',
                marker_color='#1D9E75', opacity=0.85, showlegend=sl,
                hovertemplate='%{x|%b %y}<br>Test: %{y:,}<extra></extra>'), row=row, col=1)
        if len(fc) > 0:
            fig.add_trace(go.Bar(x=fc['ds'], y=fc['fitted'], name='Forecast',
                marker_color='#f0883e', opacity=0.9, showlegend=sl,
                hovertemplate='%{x|%b %y}<br>Forecast: %{y:,}<extra></extra>'), row=row, col=1)
            fig.add_vrect(x0=str(fc['ds'].iloc[0]), x1=str(fc['ds'].iloc[-1]),
                fillcolor='rgba(240,136,62,0.05)', layer='below', line_width=0, row=row, col=1)
        if show_trend:
            try:
    ...
    sv = float(pmdf['trend_slope']); iv = float(pmdf['trend_intercept'])
    teq = f'{sv:+.1f}x + {iv:.0f}' if iv >= 0 else f'{sv:+.1f}x - {abs(iv):.0f}'
    profile_rows = [
        ('Demand Pattern',  html.Span(pt,  style={'fontWeight': '600', 'color': pc,   'fontSize': '12px'})),
